...
/*
let elements = document.querySelectorAll('.siteVersion');
for (let i = 0; i < elements.length; i++) {

    if (elements[i].href) {
        let targetHref = elements[i].href;
		targetHref += (targetHref.match(/\?/) ? '&' : '?') + siteVersion;
        elements[i].href=targetHref;
		console.log(targetHref);
    } else if (elements[i].src) {
        let targetSrc = elements[i].src;
		targetSrc += (targetSrc.match(/\?/) ? '&' : '?') + siteVersion;
		elements[i].src=targetSrc;
